...
public:
  RadarEsdfMapper() 
    : nh_("~"),
    tsdf_server_(nh_, nh_, tsdf_config, integrator_config, voxblox::MeshIntegratorConfig())
  {
    ...
    // 配置ESDF服务器
    // tsdf_server_.setTraversabilityRadius(1.2);  //机器人或无人机半径
    // tsdf_server_.setClearSphere(true);
    // tsdf_server_.setEsdfMaxDistance(max_distance_);

    tsdf_integrator_ = std::make_shared<voxblox::FastTsdfIntegrator>(  //voxblox::SimpleTsdfIntegrator
        integrator_config, 
        tsdf_server_.getTsdfMapPtr()->getTsdfLayerPtr()
    );    
...
private:
  void pointcloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    tsdf_server_.getTsdfMapPtr()->getTsdfLayerPtr()->removeAllBlocks();
    ...
    tsdf_server_.publishPointclouds();
